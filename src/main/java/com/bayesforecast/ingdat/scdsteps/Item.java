...
public class Item implements Comparable<Item>{
	...
	public String addState(Date date, State state) throws StateConflictException{
		...
		if(this.states.containsKey(date)){
			...
			boolean insert = true;
			// 2. Si est� vac�o, se inserta el nuevo estado y listo
			if(!this.states.isEmpty()){
				retorno += "\t\tNo est� vac�o\n";
				// 2.1 Si no lo est�, se hacen las comprobaciones
				...
				// 2.3 Si no es el �ltimo, se comprueban los punteross por la derecha
				if(next != null){
	...
	public int compareTo(Item o) {
		...
		int result = 0;
		// 1. Comprobaci�n del tama�o del array
		if(id.size() == this.id.size()){
			// 2. Comparaci�n objeto a objeto.
